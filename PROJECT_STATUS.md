...
1. **일정 투표/확정/변경/취소 등 실시간 반영 및 UI/UX 마감**
2. **모임 생성/수정/삭제 시 Firestore 데이터 구조 및 동기화 최적화**
3. **채팅방 기능(사진, 공지, 실시간 메시지) 안정화 및 UX 개선**
4. **Expo Push Notification 연동 및 주요 이벤트 푸시 알림**
5. **프로필 편집/설정 기능 고도화**
6. **Firestore 보안 규칙 강화, 에러 로깅/모니터링 도입**
7. **로딩/스켈레톤/에러 등 세부 UX 마감, 반응형/접근성/애니메이션**
8. **단위 테스트(Jest), E2E 테스트(Detox), 앱스토어 배포 준비**
